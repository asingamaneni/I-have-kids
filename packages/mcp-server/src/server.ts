...
export function createMcpServer(service = createLocalService()): McpServer {
  ...
  register("generate_activity", "Generate deterministic original practice for an active curriculum concept and configured learning stage. When studentId is given, the concept and stage must already be available on that student's learning path (call get_learning_path or recommend_next_activity first); locked concepts are refused with their unmet prerequisites. Follow with validate_and_store_activity to place the activity on the shelf.", z.object({ subject: z.string().min(1).optional(), conceptId: studentId.optional(), generator: studentId.optional(), seed: z.number().int(), studentId: studentId.optional(), itemCount: z.number().int().min(1).max(40).optional(), representationStage: z.string().min(1).optional(), difficultyLevel: z.number().int().min(0).max(10).optional(), now }), async (input: any) => service.generateActivity(input));
