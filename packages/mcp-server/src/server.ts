...
export function createMcpServer(service = createLocalService()): McpServer {
  ...
  register("initialize_demo", "Seed the local child-learning demo idempotently; never resets existing data.", z.object({ now }), async ({ now: at }: { now?: string }) => service.initializeDemo(at));
  ...
  register("create_student", "Create a local learner profile. Age and school placement adapt presentation but never limit the learning graph.", z.object({ id: studentId.optional(), displayName: z.string().min(1), birthDate: z.string().date().optional(), schoolPlacement: z.string().min(1).optional(), preferredLanguage: z.string().min(2).optional(), accommodations: z.array(z.string().min(1)).optional(), interests: z.array(z.string().min(1)).optional(), learningGoals: z.array(z.string().min(1)).optional(), selectedSubjects: z.array(z.string().min(1)).optional(), reportedCapabilities: z.array(z.string().min(1)).optional(), baselineNotes: z.string().min(1).optional(), baselineStatus: z.enum(["unassessed", "diagnostic-in-progress", "established"]).optional() }), async (input: any) => service.createStudent(input));
