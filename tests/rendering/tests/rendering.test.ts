...
import { generateAdditionWithinTen, generateEnglishBeginningSounds, generateSubtractionWithinTen, generateEqualGroups, generateFairSharing, generateHandwritingWriting, generateReadingForDetail, generateReasoning, generateScienceObservation } from "../../../packages/domain/src/index.ts";
...
import { WorksheetRenderer, worksheetOptions, worksheetPrintChunks, worksheetTemplateFor } from "../../../packages/rendering/src/index.ts";
...
describe("worksheet rendering contracts", () => {
  ...
  it("selects semantic templates for the broader packs", () => {
    ...
    expect(html).toContain("same amount in each box");
  ...
  it("creates stable Letter page sections and an isolated adult answer page", () => {
    ...
    expect(worksheetPrintChunks(activity.items).map((page) => page.length)).toEqual([3, 4, 3]);
    const html = renderToStaticMarkup(createElement(WorksheetRenderer, { activity, options: { mode: "print", showAnswers: true } }));
    expect((html.match(/class="worksheet-page(?:"| )/g) ?? []).length).toBe(4);
    expect(html).toContain("Page 1 of 4");
    expect(html).toContain("Page 4 of 4");
    expect(html.indexOf("worksheet-answer-page")).toBeGreaterThan(html.indexOf("Page 3 of 4"));
  });
});
