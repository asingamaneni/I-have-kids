...
1. Parse `$ARGUMENTS` or the caller's structured request for subject, concept, difficulty, representation stage, activity type, question count, duration, visual support, and student id. Load the student's local learning path, active adult directives, concept state, and recent history through the available local MCP tools.
...
4. Produce the structured activity specification first. Validate it against the local contract, answer specs, requested concept, and progression policy before asking the local renderer to create an artifact.
