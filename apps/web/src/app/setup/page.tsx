...
export default async function SetupPage() {
  ...
  const subjects: SetupSubject[] = curriculum.subjects.map((subject) => ({ ...subject, capabilities: concepts.filter((concept) => concept.subject === subject.id).flatMap((concept) => concept.assessmentClaims.map((id) => ({ id, title: capabilityTitle(id, concept.title) }))) }));
