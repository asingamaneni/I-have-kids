...
export async function POST(request: Request) {
  try {
    ...
    const stored = await generateAndStoreActivity({ studentId: body.studentId, seed: body.seed ?? Date.now(), ...(body.subject === undefined ? {} : { subject: body.subject }), ...(body.conceptId === undefined ? {} : { conceptId: body.conceptId }), ...(body.generator === undefined ? {} : { generator: body.generator }), ...(body.itemCount === undefined ? {} : { itemCount: body.itemCount }), ...(body.representationStage === undefined ? {} : { representationStage: body.representationStage }) });
