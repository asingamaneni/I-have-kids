...
import { saveDigitalSubmission, type DataScope } from "@/lib/data";

export async function POST(request: Request) {
  try {
    const body = await request.json() as Record<string, unknown>;
    const scope: DataScope = body.dataScope === "demo" ? "demo" : "household";
    const submissionInput = { ...body };
    delete submissionInput.dataScope;
    const parsed = SubmissionSchema.parse(submissionInput);
    const result = await saveDigitalSubmission(parsed, scope);
