...
export default async function CurriculumPage({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  const curriculum = await getCurriculumOverview() as { activeRevisions: CurriculumPackRevision[]; subjects: Array<{ id: string; title: string; description: string }>; proposals: Array<{ proposal: CurriculumRevisionProposal; decision?: CurriculumRevisionDecision; active?: boolean }> };
