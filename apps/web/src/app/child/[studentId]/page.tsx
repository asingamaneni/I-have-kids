...
import { toChildLearnerRoadmap, type LearnerRoadmap } from "@child-learning/contracts";
...
export default async function ChildHome({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  return <AppShell student={data.student}><main className="content-wrap child-home">
    ...
    <section className="child-hero"><div><p className="eyebrow">Ready when you are</p><h2>{latest?.title ?? "A fresh practice page"}</h2><p>{latest?.objectives[0] ?? "Try a little learning today."}</p><Link className="button button-primary" href={latest ? `/child/${studentId}/activity/${latest.id}` : "#"}>{latest ? "Start practice" : "No activity yet"}</Link></div><div className="hero-doodle roadmap-doodle" aria-hidden="true"><span>Done</span><b>→</b><strong>Now</strong><b>→</b><span>Next</span></div></section>
