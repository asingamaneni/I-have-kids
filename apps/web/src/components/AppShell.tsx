...
export function AppShell({ children, student, adult = false }: { children: ReactNode; student?: Student; adult?: boolean }) {
  ...
  return <div className={`app-frame ${adult ? "adult-frame" : "child-frame"}`}>
    ...
    {adult && student && <nav className="adult-nav" aria-label="Adult navigation"><Link href={`/adult/${student.id}`}>Overview</Link><Link href={`/adult/${student.id}/progress`}>Progress</Link><Link href={`/adult/${student.id}/path`}>Learning path</Link><Link href={`/adult/${student.id}/history`}>History</Link><Link href={`/adult/${student.id}/worksheets`}>Worksheets</Link><Link href={`/adult/${student.id}/reviews`}>Reviews</Link><Link href={`/adult/${student.id}/reports`}>Reports</Link><Link href={`/adult/${student.id}/settings`}>Settings</Link></nav>}
    {children}
  </div>;
}
