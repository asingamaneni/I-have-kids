...
import type { ConceptAvailability } from "@kindergarten/contracts";
...
export function LearningPathView({ studentId, concepts }: { studentId: string; concepts: ConceptAvailability[] }) {
  ...
  async function apply(concept: ConceptAvailability, action: "introduce" | "prioritize" | "defer" | "clear") {
    ...
    const response = await fetch("/api/learning-directives", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ studentId, conceptId: concept.conceptId, action, reason, requestedStage: action === "introduce" ? "concrete" : undefined, priority: action === "prioritize" ? 4 : undefined }) });
