...
import { ActivitySpecSchema, ChildActivitySpecSchema, SubmissionSchema, StudentSchema, toChildActivitySpec, type ActivitySpec, type CapabilityClaim, type ChildActivitySpec, type ConceptAvailability, type Student, type StudentCreateRequest, type Submission } from "@kindergarten/contracts";
import { comparabilityKey, filterAvailableActivities } from "@kindergarten/domain";
import { createLocalService, type LocalService } from "@kindergarten/mcp-server";
import { normalizeImage } from "@kindergarten/storage";

type ServiceWork<T> = (service: LocalService) => Promise<T> | T;

type BaselineTarget = { subject: ActivitySpec["subject"]; conceptId: string; stage: "pictorial" | "abstract"; rank: number };
const BASELINE_TARGETS: Record<CapabilityClaim, BaselineTarget> = {
...
function toStudent(row: Record<string, unknown> | undefined): Student | undefined {
  ...
  let metadata: { preferredLanguage?: string; accommodations?: string[]; reportedCapabilities?: CapabilityClaim[]; baselineNotes?: string; baselineStatus?: Student["baselineStatus"] } = {};
  try { metadata = JSON.parse(String(row.metadata_json ?? "{}")) as typeof metadata; } catch { metadata = {}; }
  return StudentSchema.parse({ id: row.id, displayName: row.display_name, birthDate: row.birth_date ?? undefined, gradeBand: row.grade ?? "early-learning", preferredLanguage: metadata.preferredLanguage ?? "en", accommodations: metadata.accommodations ?? [], reportedCapabilities: metadata.reportedCapabilities ?? [], ...(metadata.baselineNotes ? { baselineNotes: metadata.baselineNotes } : {}), baselineStatus: metadata.baselineStatus ?? "unassessed", createdAt: row.created_at, updatedAt: row.updated_at });
}
...
export async function createStudentWithStarter(input: StudentCreateRequest) {
  return withService(async (service) => {
    ...
    for (const target of targetBySubject.values()) {
      service.applyLearningDirective({ id: `baseline-${student.id}-${target.conceptId}`, studentId: student.id, conceptId: target.conceptId, action: "assess", reason: "Adult-reported current capability; verify with a short starting assessment before changing progress.", authorId: "local-adult", requestedStage: target.stage });
...
export async function getAvailableChildActivities(studentId: string) {
  return withService((service) => {
    ...
    return { student, activities: filterAvailableActivities(activities, learningPath.availability).map(toChildActivitySpec), learningPathVersion: learningPath.curriculumVersion };
  });
}
...
export async function getStudentBundle(studentId: string) {
  return withService((service) => {
    ...
    const availableActivities = filterAvailableActivities(activities, learningPath.availability);
    ...
    return { student, activities, availableActivities, learningPath, states, submissions, evaluations, pendingEvaluations, timeline, reports, recommendations };
  });
}
...
export async function getChildActivity(activityId: string): Promise<ChildActivitySpec | undefined> {
  return withService((service) => {
    try {
      ...
      if (filterAvailableActivities([adultActivity], learningPath.availability).length === 0) return undefined;
...
export async function generateAndStoreActivity(input: { subject?: "math" | "english" | "reasoning" | "science"; conceptId?: string; generator?: string; seed: number; studentId: string; itemCount?: number; representationStage?: "concrete" | "pictorial" | "abstract" }) {
  return withService(async (service) => {
...
export async function applyLearningDirective(input: { id: string; studentId: string; conceptId: string; action: "introduce" | "assess" | "prioritize" | "defer" | "clear"; reason: string; authorId: string; requestedStage?: "concrete" | "pictorial" | "abstract"; priority?: number; expiresAt?: string }) {
  return withService(async (service) => {
    const result = service.applyLearningDirective(input);
    ...
    const representationStage = input.action === "introduce" ? input.requestedStage ?? "concrete" : concept.currentStage;
